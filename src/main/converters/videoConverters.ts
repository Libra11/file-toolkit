...
async function convertToMp4(
  ...
  try {
    console.log(`开始转换${inputFormat}到MP4: ${inputPath} -> ${outputPath}`)
    ...
    console.log(`${inputFormat}到MP4转换成功: ${outputPath}`)
    return outputPath
  } catch (error) {
    console.error(`${inputFormat}到MP4转换错误:`, error)
    throw error
  }
}
...
export async function convertAviToMp4(inputPath: string, outputPath: string): Promise<string> {
  return convertToMp4(inputPath, outputPath, 'AVI')
}
...
export async function convertMovToMp4(inputPath: string, outputPath: string): Promise<string> {
  return convertToMp4(inputPath, outputPath, 'MOV')
}
...
export async function convertWebmToMp4(inputPath: string, outputPath: string): Promise<string> {
  return convertToMp4(inputPath, outputPath, 'WEBM')
}
