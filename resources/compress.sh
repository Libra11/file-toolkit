...
# 获取总文件数，并确保是纯数字
total_files=$(ls -1 "${INPUT_DIR}"*.{JPEG,JPG,PNG,jpeg,jpg,png} 2>/dev/null | wc -l | tr -d ' ')
# 确保是正整数
total_files=$(($total_files + 0))
current=0

echo "开始压缩图片，总文件数: ${total_files}"
...
do
    ...
    # 使用printf精确格式化进度信息
    printf "处理进度: [%d%%] (%d/%d) 当前文件: %s\n" $percentage $current $total_files "$filename"
done

# 完成后换行
echo "所有图像已成功调整大小并保存到 $OUTPUT_DIR"